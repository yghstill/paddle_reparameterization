...
class MobileOneBlock(nn.Layer):
    ...
    def __init__(self,
        ...
        if kernel_size > 1:
            ...
            if self.small_kernel > 1:
                self.rbr_scale1 = ConvBNLayer(
                    self.in_channels,
                    self.out_channels,
                    self.small_kernel - 2,
                    stride=self.stride,
    ...
    def _get_kernel_bias(self):
        ...
        if self.rbr_scale is not None:
            ...
            padding_size = (self.kernel_size - self.small_kernel) // 2
        ...
        if self.rbr_scale1 is not None:
            ...
            # Pad scale branch kernel to match conv branch kernel size. 1x1->3x3
            padding_size = (self.kernel_size - self.small_kernel + 2) // 2
            kernel_scale1 = paddle.nn.functional.pad(
                kernel_scale1,
                [padding_size, padding_size, padding_size, padding_size])
            kernel_scale += kernel_scale1
            bias_scale += bias_scale1
...
class RMNet(nn.Layer):
    ...
    def __init__(self,
        ...
        self.stage1 = self._make_stage(
            int(64 * width_multipliers[1]),
            num_blocks_per_stage[0],
            num_se_blocks=0)
        self.stage2 = self._make_stage(
            int(128 * width_multipliers[2]),
            num_blocks_per_stage[1],
            num_se_blocks=0)
        self.stage3 = self._make_stage(
            int(256 * width_multipliers[3]),
            num_blocks_per_stage[2],
            num_se_blocks=int(num_blocks_per_stage[2] // 2) if use_se else 0)
        self.stage4 = self._make_stage(
            int(512 * width_multipliers[4]),
        ...
        if self.use_last_conv:
            ...
            self.last_conv = MobileOneBlock(
                    in_channels=int(512 * width_multipliers[4]),
                    out_channels=self.class_expand,
                    kernel_size=1,
                    stride=1,
                    num_conv_branches=self.num_conv_branches)
            self.dropout = nn.Dropout(p=self.dropout_prob, mode="downscale_in_infer")
            self.linear = nn.Linear(self.class_expand, class_num)
        else:
            self.linear = nn.Linear(int(512 * width_multipliers[4]), class_num)
...
PARAMS = {
    "s0": {
        "width_multipliers": (0.75, 0.75, 1.0, 1.0, 2.0),
        "num_conv_branches": 4
    },
    "s1": {
        "width_multipliers": (1.5, 1.5, 1.5, 2.0, 2.5)
    },
    "s2": {
        "width_multipliers": (1.5, 1.5, 2.0, 2.5, 4.0)
    },
    "s3": {
        "width_multipliers": (2.0, 2.0, 2.5, 3.0, 4.0)
    },
    "s4": {
        "width_multipliers": (3.0, 3.0, 3.5, 3.5, 4.0),
        "use_se": True
    },
}
